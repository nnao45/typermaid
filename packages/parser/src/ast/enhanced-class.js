 ...
 */
export class EnhancedClassDiagramAST {
    // AST properties
    type = 'ClassDiagram';
    diagram;
    loc;
     ...
     */
    addAttribute(classId, name, type, visibility = '+') {
     ...
     * @returns this for method chaining
     */
    addMethod(classId, name, returnType = 'void', parameters = [], visibility = '+') {
        const className = classId;
        const cls = this.diagram.classes.find((c) => c.name === className);
        if (cls) {
     ...
     * @returns this for method chaining
     */
    addRelation(from, to, type, label, cardinalityFrom, cardinalityTo) {
     ...
     * @returns this for method chaining
     */
    addImplementation(implementer, interface_) {
     ...
     * @returns this for method chaining
     */
    addAssociation(from, to, label, cardinalityFrom, cardinalityTo) {
    ...
    asCode() {
        ...
        for (const cls of this.diagram.classes) {
            ...
            for (const member of cls.members) {
                ...
                if (member.type === 'attribute') {
                    ...
                    lines.push(`    ${cls.name} : ${isStatic}${isAbstract}${visibility}${member.name}(${params}) ${returnType}`);
                }
            }
        }
        ...
        for (const rel of this.diagram.relations) {
            ...
            const label = rel.label
                ? ` : ${typeof rel.label === 'string' ? rel.label : rel.label.raw}`
                : '';
...
//# sourceMappingURL=enhanced-class.js.map