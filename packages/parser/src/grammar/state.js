...
export class StateParser {
    ...
    parseDirection() {
        ...
        if (this.check('TB') ||
            ...
            throw new ParserError('Expected direction value', this.peek().start.line, this.peek().start.column);
        }
    ...
    checkTransition() {
        ...
        if (this.check('SQUARE_OPEN') || this.check('IDENTIFIER')) {
            ...
            this.skipWhitespace();
            // Check for arrow (-->, --)
            const hasArrow = this.check('ARROW') ||
                this.check('LINE') ||
                this.check('MINUS') ||
                this.peek().value === '-';
            this.current = saved;
