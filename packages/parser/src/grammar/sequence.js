...
export class SequenceParser {
    ...
    parseAlt() {
        ...
        while (!this.isAtEnd() && !this.match('ELSE') && !this.match('END')) {
            this.skipNewlines();
            if (this.match('ELSE') || this.match('END'))
                break;
    ...
    parsePar() {
        ...
        while (!this.isAtEnd() && !this.match('AND') && !this.match('END')) {
            this.skipNewlines();
            if (this.match('AND') || this.match('END'))
                break;
            const stmt = this.parseStatementInBlock();
            if (stmt)
                statements.push(stmt);
    ...
    parseCritical() {
        ...
        while (!this.isAtEnd() && !this.match('OPTION') && !this.match('END')) {
            this.skipNewlines();
            if (this.match('OPTION') || this.match('END'))
                break;
            const stmt = this.parseStatementInBlock();
            if (stmt)
                statements.push(stmt);
    ...
    consume(type) {
        const token = this.peek();
        if (token.type !== type) {
            throw new ParserError(`Expected ${type}, but got ${token.type}`, token.start.line, token.start.column);
        }
        return this.advance();
    }
    isAtEnd() {
