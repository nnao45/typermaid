...
export class ERParser {
    ...
    parseRelationshipNotation() {
        ...
        if (notation.length < 5) {
            throw new ParserError(`Invalid relationship notation '${notation}' (expected at least 5 characters)`, this.peek().start.line, this.peek().start.column);
        }
    ...
    parseCardinalityString(str) {
        switch (str) {
            ...
            case 'o|':
                return 'ZERO_OR_ONE';
            default:
                throw new ParserError(`Expected cardinality (||, }|, }o, o{, |{, |o, o|), got '${str}'`, this.peek().start.line, this.peek().start.column);
        }
    }
    parseIdentificationString(str) {
        ...
        throw new ParserError(`Expected identification (-- or ..), got '${str}'`, this.peek().start.line, this.peek().start.column);
    }
    lookAheadForEntity() {
