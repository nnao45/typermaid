 ...
 */
function generateNodeElement(node, theme) {
 ...
 */
function generateEdgeElement(edge, theme) {
  const elements = [];
 ...
 */
function generateMarkerDefs(theme) {
 ...
 */
export function generateFlowchartSVG(diagram, theme = DEFAULT_THEME) {
  // Calculate layout
  const layout = createLayout(diagram);
  // Generate elements
  const nodeElements = layout.nodes.map((node) => generateNodeElement(node, theme));
  const edgeElements = layout.edges.map((edge) => generateEdgeElement(edge, theme));
