 ...
 */
export function straightLine(from: { x: number; y: number }, to: { x: number; y: number }): string {
 ...
 */
export function roundedPath(points: Array<{ x: number; y: number }>, radius: number = 5): string {
  if (points.length < 2) return '';
  if (points.length === 2) {
    const [p0, p1] = points;
    if (!p0 || !p1) return '';
 ...
 */
export function calculateLabelPosition(points: Array<{ x: number; y: number }>): {
  x: number;
  y: number;
} {
