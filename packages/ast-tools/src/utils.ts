...
export function mergeDiagrams(diagram1: ERDiagramAST, diagram2: ERDiagramAST): ERDiagramAST;
export function mergeDiagrams(
...
export function mergeDiagrams(
  ...
  switch (diagram1.type) {
    case 'FlowchartDiagram':
      return {
        ...diagram1,
        body: [...diagram1.body, ...(diagram2 as FlowchartDiagramAST).body],
    ...
    case 'StateDiagram':
      return {
        ...
        diagram: {
          ...diagram1.diagram,
          states: [...diagram1.diagram.states, ...(diagram2 as StateDiagramAST).diagram.states],
          transitions: [
    ...
    case 'ClassDiagram':
      return {
        ...
        diagram: {
          ...diagram1.diagram,
          classes: [...diagram1.diagram.classes, ...(diagram2 as ClassDiagramAST).diagram.classes],
          relations: [
    ...
    case 'ERDiagram':
      return {
        ...
        diagram: {
          ...diagram1.diagram,
          entities: [...diagram1.diagram.entities, ...(diagram2 as ERDiagramAST).diagram.entities],
          relationships: [
...
export function removeNode(ast: ProgramAST, nodeId: string): ProgramAST {
  return transformAST(ast, {
    ...
    Edge: (edge) => (edge.from === nodeId || edge.to === nodeId ? null : edge),
    Participant: (node) => (node.id === nodeId ? null : node),
    Message: (msg) => (msg.from === nodeId || msg.to === nodeId ? null : msg),
    State: (node) => (node.id === nodeId ? null : node),
    Transition: (trans) => (trans.from === nodeId || trans.to === nodeId ? null : trans),
  });
}
...
export function getAllNodeIds(diagram: FlowchartDiagramAST): string[] {
  ...
  function collectIds(body: readonly (FlowchartNodeAST | EdgeAST | { type: string })[]): void {
    for (const item of body) {
      if (item.type === 'Node') {
        ids.push((item as FlowchartNodeAST).id);
      } else if (item.type === 'Subgraph' && 'body' in item && Array.isArray(item.body)) {
 ...
 * ```
 */
export function validateFlowchart(diagram: FlowchartDiagramAST): ValidationResult {
  const nodeIds = new Set(getAllNodeIds(diagram));
  const errors: string[] = [];
  const warnings: string[] = [];

  function checkEdges(body: readonly (FlowchartNodeAST | EdgeAST | { type: string })[]): void {
    for (const item of body) {
      if (item.type === 'Edge') {
  ...
  function collectConnectedNodes(
    ...
    for (const item of body) {
      if (item.type === 'Edge') {
        ...
        collectConnectedNodes(item.body as (FlowchartNodeAST | EdgeAST | { type: string })[]);
      }
    }
  }
 ...
 * ```
 */
export function validateSequence(diagram: SequenceDiagramAST): ValidationResult {
  ...
  function checkMessages(
    ...
    for (const stmt of statements) {
      if (stmt.type === 'message') {
        ...
        checkMessages(
          (stmt as { statements: { type: string; from?: string; to?: string }[] }).statements
        );
      }
    }
  }
