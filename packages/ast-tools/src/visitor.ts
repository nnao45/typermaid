...
export class ASTTransformer {
  ...
  private visit<T>(node: T, visitor: ASTVisitor): T {
    ...
    switch (astNode.type) {
      case 'FlowchartDiagram':
        return this.visitFlowchartDiagram(astNode as FlowchartDiagramAST, visitor) as T;
      case 'Node':
        return (visitor.visitFlowchartNode?.(astNode as FlowchartNodeAST) ?? astNode) as T;
      case 'Edge':
      ...
      case 'SequenceDiagram':
        return this.visitSequenceDiagram(astNode as SequenceDiagramAST, visitor) as T;
      case 'Participant':
        return (visitor.visitParticipant?.(astNode as Participant) ?? astNode) as T;
      case 'Message':
      ...
      case 'StateDiagram':
        return this.visitStateDiagram(astNode as StateDiagramAST, visitor) as T;
      case 'State':
        return (visitor.visitState?.(astNode as unknown as State) ?? astNode) as T;
      case 'Transition':
        return (visitor.visitTransition?.(astNode as unknown as Transition) ?? astNode) as T;
      case 'StateNote':
        return (visitor.visitStateNote?.(astNode as unknown as StateNote) ?? astNode) as T;

      case 'ClassDiagram':
        return this.visitClassDiagram(astNode as ClassDiagramAST, visitor) as T;
      case 'Class':
        return this.visitClass(astNode as unknown as Class, visitor) as T;
      case 'ClassMember':
        return (visitor.visitClassMember?.(astNode as unknown as ClassMember) ?? astNode) as T;
      case 'Relationship':
        return (visitor.visitRelationship?.(astNode as unknown as Relationship) ?? astNode) as T;

      case 'ERDiagram':
        return this.visitERDiagram(astNode as ERDiagramAST, visitor) as T;
      case 'Entity':
        return this.visitEntity(astNode as unknown as Entity, visitor) as T;
      case 'Attribute':
      ...
      case 'GanttTask':
        return (visitor.visitGanttTask?.(astNode as unknown as GanttTask) ?? astNode) as T;
      case 'GanttSection':
        return (visitor.visitGanttSection?.(astNode as unknown as GanttSection) ?? astNode) as T;
  ...
  private visitFlowchartDiagram(
    ...
    const transformedBody = node.body.map((item) => this.visit(item, visitor)) as (
      | FlowchartNodeAST
      | EdgeAST
      | SubgraphAST
    )[];
  ...
  private visitSequenceDiagram(node: SequenceDiagramAST, visitor: ASTVisitor): SequenceDiagramAST {
    const transformedStatements = node.diagram.statements.map((stmt) => this.visit(stmt, visitor));
  ...
  private visitLoop(node: Loop, visitor: ASTVisitor): Loop {
    const transformedStatements = node.statements.map((stmt) => this.visit(stmt, visitor));
  ...
  private visitStateDiagram(node: StateDiagramAST, visitor: ASTVisitor): StateDiagramAST {
    const transformedStates = node.diagram.states.map((state) => this.visit(state, visitor));

    const transformedTransitions = node.diagram.transitions.map((transition) =>
      this.visit(transition, visitor)
    );

    const transformedNotes = node.diagram.notes?.map((note) => this.visit(note, visitor));
  ...
  private visitClassDiagram(node: ClassDiagramAST, visitor: ASTVisitor): ClassDiagramAST {
    const transformedClasses = node.diagram.classes.map((cls) => this.visit(cls, visitor));

    const transformedRelationships = node.diagram.relations.map((rel) => this.visit(rel, visitor));
  ...
  private visitClass(node: Class, visitor: ASTVisitor): Class {
    const transformedMembers = node.members.map((member) => this.visit(member, visitor));
  ...
  private visitERDiagram(node: ERDiagramAST, visitor: ASTVisitor): ERDiagramAST {
    const transformedEntities = node.diagram.entities.map((entity) => this.visit(entity, visitor));
  ...
  private visitEntity(node: Entity, visitor: ASTVisitor): Entity {
    const transformedAttributes = node.attributes.map((attr) => this.visit(attr, visitor));
  ...
  private visitGanttDiagram(node: GanttDiagramAST, visitor: ASTVisitor): GanttDiagramAST {
