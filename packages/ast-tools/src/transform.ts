 ...
 * ```
 */
export function transformAST<T extends ProgramAST>(ast: T, transforms: TransformFunctions): T {
...
function walkAST<T>(node: T, transforms: TransformFunctions): T {
  ...
  switch (astNode.type) {
    ...
    case 'Break':
      ...
      break;
    case 'State':
      result = transforms.State?.(astNode as unknown as State) ?? astNode;
      break;
    case 'Transition':
      result = transforms.Transition?.(astNode as unknown as Transition) ?? astNode;
      break;
    case 'StateNote':
      result = transforms.StateNote?.(astNode as unknown as StateNote) ?? astNode;
      break;
    case 'Class':
      result = transforms.Class?.(astNode as unknown as Class) ?? astNode;
      break;
    case 'ClassMember':
      result = transforms.ClassMember?.(astNode as unknown as ClassMember) ?? astNode;
      break;
    case 'Relationship':
      result = transforms.Relationship?.(astNode as unknown as Relationship) ?? astNode;
      break;
    case 'Entity':
      result = transforms.Entity?.(astNode as unknown as Entity) ?? astNode;
      break;
    case 'Attribute':
      result = transforms.Attribute?.(astNode as unknown as Attribute) ?? astNode;
      break;
    case 'ERRelationship':
      result = transforms.ERRelationship?.(astNode as unknown as ERRelationship) ?? astNode;
      break;
    case 'GanttTask':
      result = transforms.GanttTask?.(astNode as unknown as GanttTask) ?? astNode;
      break;
    case 'GanttSection':
      result = transforms.GanttSection?.(astNode as unknown as GanttSection) ?? astNode;
      break;
    default:
      result = astNode;
  }
  ...
  if (
    ...
    return {
      ...withBody,
      body: withBody.body.map((item) => walkAST(item, transforms)).filter((item) => item !== null),
    } as T;
  }
 ...
 * ```
 */
export function findNodes<T extends { type: string }>(ast: ProgramAST, type: T['type']): T[] {
  const results: T[] = [];
  ...
  function collect(node: unknown): unknown {
    ...
    if ('classes' in astNode && Array.isArray((astNode as { classes: unknown[] }).classes)) {
      for (const item of (astNode as { classes: unknown[] }).classes) {
        collect(item);
      }
    }

    if ('entities' in astNode && Array.isArray((astNode as { entities: unknown[] }).entities)) {
      for (const item of (astNode as { entities: unknown[] }).entities) {
        collect(item);
      }
    }

    if ('sections' in astNode && Array.isArray((astNode as { sections: unknown[] }).sections)) {
...
export function replaceNodeById<T extends { id: string }>(
  ...
  return transformAST(ast, {
    FlowchartNode: (node) => (node.id === id ? { ...node, ...newNode } : node),
    Participant: (node) => (node.id === id ? { ...node, ...newNode } : node),
    State: (node) => (node.id === id ? { ...node, ...newNode } : node),
...
export function replaceNodeByName<T extends { name: string }>(
  ...
  return transformAST(ast, {
    Class: (node) => (node.name === name ? { ...node, ...newNode } : node),
    Entity: (node) => (node.name === name ? { ...node, ...newNode } : node),
  } as TransformFunctions);
}
