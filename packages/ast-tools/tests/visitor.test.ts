...
sequenceDiagram
  ...
  A->>B: Hello
      `.trim();
			...
			const participants = (
				transformed as { statements: ParticipantAST[] }
			).statements.filter((item) => item.type === "Participant") as ParticipantAST[];
...
sequenceDiagram
  ...
  A->>B: Hello World
      `.trim();
			...
			const messages = (
				transformed as { statements: MessageAST[] }
			).statements.filter((item) => item.type === "Message") as MessageAST[];
  ...
  subgraph sub1
    ...
    B[end]
      ...
      `.trim();
			...
			const subgraph = (transformed as { body: { type: string; body: FlowchartNodeAST[] }[] }).body.find(
				(item) => item.type === "Subgraph",
			);

			const nodes = subgraph?.body.filter(
				(item) => item.type === "Node",
			) as FlowchartNodeAST[];
