...
import type { FlowchartNodeAST, EdgeAST, MessageAST } from "@typermaid/parser";
import {
	transformAST,
	findNodes,
	replaceNodeById,
...
flowchart LR
  A[Start] --> B[Process]
      `.trim();
			...
			const nodes = diagram.body.filter(
				(item) => item.type === "Node",
			) as FlowchartNodeAST[];
...
flowchart LR
  ...
  C --> A
      `.trim();
			...
			const transformed = transformAST(ast, {
				Edge: (edge) =>
					edge.from === "C" && edge.to === "A" ? null : edge,
			});

			const diagram = transformed.body[0] as { body: EdgeAST[] };
			const edges = diagram.body.filter(
				(item) => item.type === "Edge",
			) as EdgeAST[];
...
sequenceDiagram
  ...
  A->>B: Hello World
      `.trim();
			...
			const messages = diagram.statements.filter(
...
sequenceDiagram
  ...
  participant Server
    `.trim();
		...
		const ast = parse(code);
		const participants = findNodes<{ type: string; id: string }>(
			ast,
			"Participant",
		);
