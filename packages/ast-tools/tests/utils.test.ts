...
stateDiagram-v2
  ...
  B --> [*]
    `.trim();
		...
		expect(
			diagram.transitions.find((t) => t.from === "A" || t.to === "A"),
		).toBeUndefined();
	});
});
...
sequenceDiagram
  participant A
    `.trim();
		...
		expect(result.valid).toBe(false);
		expect(result.errors).toContain(
			"Message references undeclared participant: Undeclared",
		);
	});
});
...
stateDiagram-v2
  [*] --> Idle
    `.trim();
		...
		expect(result.valid).toBe(false);
		expect(result.errors).toContain(
			"Transition references non-existent state: NonExistent",
		);
	});
});
